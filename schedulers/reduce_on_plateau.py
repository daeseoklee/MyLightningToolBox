...
class WarmupAndReduceLROnPlateau(ReduceLROnPlateau):
    """
    """
    ...
    def __init__(self, 
                 ...
                 begin_lr : float = 1e-10,
                 mode='min', # This and the following are superclass kwargs
                 factor=0.1,
                 patience=10,
                 threshold=1e-4,
                 threshold_mode='rel',
    ...
    def step(self, on_epoch_end=False, metrics=None):
        ...
        -After each gradient step, run self.step(on_epoch_end=False)
        -At the end of each epoch, run self.step(on_epoch_end=True, metrics=loss)
        """
        if on_epoch_end:
            if self.warmingup:
                return 
